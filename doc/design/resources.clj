...
;; El problema con los CSS
;; Los componentes están íntimamente ligados a sus archivos SCSS. Ahora mismo esto no es evidente en el
;; código. ventas usa un sistema basado en css modules.

;; Esto no es suficiente:

(defstyle style
  [".ventas" {}
    ["&.preheader" {:background-color "#ecf0f1"
                                 :padding "8px 0px"}
                  ["> div" {:display "table"}]]])

;; Ahora mismo solo ".ventas" es transformado a una clase con un gensym, y la clase generada es
;; accesible desde un map. Aunque me gustaría que se transformase también ".preheader", transformar
;; todas las clases encontradas no es práctico, y además yo podría querer incluir una clase generada
;; en otro namespace, así que propongo lo siguiente:

(defstyle style
  [ventas.core/css-ventas {}
    ["&.preheader" {:background-color "#ecf0f1"
                    :padding "8px 0px"}
      ["> div" {:display "table"}]]])
;; Aquí el símbolo se refiere a una string con el valor ".ventas"

(defstyle style
  [ventas.core/css-ventas {}
    [(str "&" (gen-class css-preheader)) {:background-color "#ecf0f1"
                                      :padding "8px 0px"}
      ["> div" {:display "table"}]]])
;; Aquí la clase es "&.preheader-PG__1", y preheader tiene el valor ".preheader-PG__1"

(defstyle style
  [(gen-class css-button) {:background-color "black"
                           :font-size "48px"}]
  [(gen-class css-fancy-button) {:background-color "blue"
                                 :composes css-button}])
;; Un ejemplo de composición: en este caso las clases serían:
;; css-button: .css-button-PG__1
;;  {:background-color "black"
;    :font-size "48px"}
;; css-fancy-button: .css-fancy-button-PG__1
